...
def create_sequences_multi(series, time_steps):
    ...
    for i in range(time_steps, len(series)):
        ...
        y.append(series[i, 0])  # Predict NIFTY50 (index 0)
...
class GraphConvLayer(Layer):
    def __init__(self, units, adj_matrix, **kwargs):
    ...
    def call(self, node_features):
        # node_features: (batch_size, n_nodes)
        # GraphConv: h' = A @ Dense(h)
        h_transformed = self.dense(node_features)  # (batch, units)
        # Expand for matrix multiplication
        batch_size = tf.shape(h_transformed)[0]
        adj_tiled = tf.tile(self.adj_matrix[None, :, :], [batch_size, 1, 1])
        output = tf.matmul(adj_tiled, tf.expand_dims(h_transformed, -1))  # (batch, n_nodes, 1)
        output = tf.reduce_mean(output, axis=[1, 2])  # Pool to (batch,)
        return tf.expand_dims(output, -1)  # (batch, 1)
...
# Graph Branch: Inter-stock correlations (last timestep)
last_timestep = tf.keras.layers.Lambda(lambda x: x[:, -1, :])(inputs)  # (batch, 5)
graph_features = GraphConvLayer(32, graph_adj)(last_timestep)  # (batch, 1)
...
axes[0,1].set_title("Last 200 Days"); axes[0,1].grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig("models/LSTM_GNN_Final/lstm_gnn_results.png", dpi=300, bbox_inches="tight")
plt.show()

# ================= SAVE EVERYTHING ================= #
results_df = pd.DataFrame({
    "Date": test_index, 
    "Actual_NIFTY50": actual_nifty, 
    "LSTM_GNN_Pred": y_pred
})
...
joblib.dump(graph_adj.numpy(), "models/LSTM_GNN_Final/correlation_graph.pkl")
